...
websocket.on("request", (request) => {
    ...
    const userId:string = (request.resourceURL.query as ParsedUrlQuery).userId as string;
    console.log("Id ",userId);
    let connections:connection = request.accept(null, request.origin);
    SocketInstance.addUser(userId,connections);
    console.log("Active Connection ", SocketInstance.ActiveConnection());
    ...
    connections.on("close", (code, des)=>{
        ...
        SocketInstance.removeUser(userId);
    })
